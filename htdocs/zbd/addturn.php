...
if ($con->query("INSERT INTO adres VALUES (NULL, '$kraj', '$miasto', '$kod', '$ulica')")){
    ...
    $ile = $ad->num_rows;
		if($ile>0){
            ...
            echo $id_adres."<br/>";
            echo "INSERT INTO turniej VALUES (NULL, '$nazwa', '$data', $id_adres, $id_dyscyplina, '$opis', '$zasady', '$transmisja', $wpisowe)";
            //if ($con->query("INSERT INTO turniej VALUES (NULL, '$data', '$id_adres', '$id_dyscyplina', '1', '$nazwa', '$opis', '$zasady', '$transmisja', '$wpisowe','100')")){
            if ($con->query(sprintf("INSERT INTO turniej VALUES (NULL, '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s')",
            mysqli_real_escape_string($con, $nazwa),
            mysqli_real_escape_string($con, $data),
            mysqli_real_escape_string($con, $id_adres),
            mysqli_real_escape_string($con, $id_dyscyplina),
            mysqli_real_escape_string($con, $opis),
            mysqli_real_escape_string($con, $zasady),
            mysqli_real_escape_string($con, $transmisja),
            mysqli_real_escape_string($con, $wpisowe)))){
